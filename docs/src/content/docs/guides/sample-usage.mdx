...
To configure the `marco` CLI, create a `marco.config.js` file at the root of your project. This file specifies how marco will fetch the performance data from the device and visualize it on dashboard. You can either use CLI [commands](api/cli/generate-report) as well.
...
The command collects data saved in device to a JSON file at specified path(`./navigation-reports/log.json`) at the root of project.
...
This command starts a local server and opens the visualization dashboard in your browser at http://localhost:8080 (or the custom port specified in `marco.config.js`).
...
The marco tool is actively used in [React Native Benchmark](https://reactnativebenchmark.dev/), a platform that benchmarks the performance of major React Native versions
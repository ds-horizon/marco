...
2. Wrap your screen with `PerformanceTracker` to track the draw time of the screen, and use `track` to send custom markers.
    ...
    ```tsx
      ...
      const MyScreen = () => {
        // Custom function to track specific actions
        const handleOnPress = () => {
          // Send a custom marker to track the event
          PerformanceTracker.track('Click', Date.now());
        };
...
4. Create a perf-tracker.config.js file at the root of your project with the following content.
...
5. Run the following command from the CLI to fetch data from the device and save it to the specified path. 
      ...
      <TabItem label="yarn" icon="seti:yarn">
        ```sh frame=none
        yarn perf-tracker generate
        ```
      </TabItem>
      <TabItem label="npm" icon="seti:npm">
        ```sh frame=none
        npm perf-tracker generate
        ```
      </TabItem>
      <TabItem label="pnpm" icon="pnpm">
        ```sh frame=none
        pnpm perf-tracker generate
        ```
      </TabItem>
    </Tabs>
...
6. To visualize the data on a dashboard, run the following command. This will open a dashboard on localhost at the desired port.
      ...
      <TabItem label="npm" icon="seti:npm">
        ```sh frame=none
        npm perf-tracker visualize
        ```
      </TabItem>
      <TabItem label="pnpm" icon="pnpm">
        ```sh frame=none
        pnpm perf-tracker visualize
        ```
      </TabItem>
    </Tabs>
</Steps>

...
To configure default values for the CLI tool, create a `marco.config.js` file in the root directory of project:
    ...
    ```
:::note
   For more details on available commands and how to override config file values, see the [generate-report](../cli/generate-report) and [visualization](../cli/visualization) documentation.
:::
